...
def test_convert_to_csv_failure(mock_convert_to_csv, caplog):
    # Arrange
    with patch("extract.make_api_get_request") as mock_make_api_request, patch(
        "extract.format_data"
    ) as mock_format_data, caplog.at_level(logging.ERROR):
...
@patch(
    "extract.create_directory_structure_and_file_name",
    side_effect=Exception("File name error"),
)
def test_create_file_name_failure(mock_create_file_name, caplog):
    # Combine all context managers into a single with statement
    with patch("extract.make_api_get_request") as mock_make_api_request, patch(
        "extract.format_data"
    ) as mock_format_data, patch(
        "extract.convert_to_csv"
    ) as mock_convert_to_csv, caplog.at_level(
        logging.ERROR
    ):
...
def test_create_s3_client_failure(mock_create_s3_client, caplog):
    ...
    with patch("extract.make_api_get_request") as mock_make_api_request, patch(
        ...
        "extract.convert_to_csv"
    ) as mock_convert_to_csv, patch(
        "extract.create_directory_structure_and_file_name"
    ) as mock_create_file_name, caplog.at_level(
        logging.ERROR
    ):
...
def test_store_in_s3_failure(mock_store_in_s3, caplog):
    ...
    with patch("extract.make_api_get_request") as mock_make_api_request, patch(
        ...
        "extract.create_s3_client"
    ) as mock_create_s3_client, caplog.at_level(
        logging.ERROR
    ):
...
def test_lambda_handler_success():
    ...
    with patch("extract.make_api_get_request") as mock_api_request, patch(
        "extract.format_data"
    ) as mock_format_data, patch(
        "extract.convert_to_csv"
    ) as mock_convert_to_csv, patch(
        ...
        "extract.store_in_s3", return_value=None
    ) as mock_store_in_s3, patch.dict(
        "os.environ", {"INGESTION_BUCKET": "mock-data-bucket"}
    ):
        ...
        mock_store_in_s3.assert_called_once_with(
            ...
            "city,temp\nLondon,20",
            "mock-data-bucket",
            "GB/London/2024/12/11/10:30.00.csv",
        )

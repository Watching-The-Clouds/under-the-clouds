...
def test_clear_weather_high_visibility():
    df = pd.DataFrame(
        {"weather_id": [800], "visibility": [10000], "visibility_description": ["high"]}
    )
...
def test_heavy_rain_low_visibility():
    df = pd.DataFrame(
        {"weather_id": [522], "visibility": [3000], "visibility_description": ["low"]}
    )
...
def test_snow_high_visibility():
    df = pd.DataFrame(
        {"weather_id": [600], "visibility": [10000], "visibility_description": ["high"]}
    )
...
def test_unknown_weather_code():
    df = pd.DataFrame(
        {"weather_id": [999], "visibility": [10000], "visibility_description": ["high"]}
    )
...
def test_multiple_weather_conditions():
    ...
    assert len(result) == 4
    assert list(result["weather_impact_code"]) == [
        "clear",
        "light_rain",
        "heavy_rain",
        "snow",
    ...
    assert all(0 <= result["speed_coefficient_low"]) and all(
        result["speed_coefficient_low"] <= 1
    )
    assert all(0 <= result["speed_coefficient_high"]) and all(
        result["speed_coefficient_high"] <= 1
    )
